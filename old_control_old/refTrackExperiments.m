...
% cost function for following a circle
r = 2;
rx = r * cos(0.5*t);
ry = r * sin(0.5*t);
cost = 100 * (px - rx)^2 + ...
...
f = Function('f',{states, u, t},{rhs, cost},{'x', 'u', 't'},{'y', 'cost'}); % fonction of the dynamics
...
for j=1:M
    [k1, k1_q] = f(x, u, tRK);
    [k2, k2_q] = f(x + DT/2 * k1, u, tRK + DT/2);
    [k3, k3_q] = f(x + DT/2 * k2, u, tRK + DT/2);
    [k4, k4_q] = f(x + DT * k3, u, tRK + DT);
    tRK = tRK + DT;
    x = x + DT/6 * (k1 + 2 * k2 +2 * k3 + k4);
    q = q + DT/6 * (k1_q + 2 * k2_q + 2 * k3_q + k4_q);
end
fd = Function('fd', {x0, Ts, u, t0RK}, {x,q}, {'x0','Ts','p','t0'}, {'xf','qf'});
...
x_prior = fd(x_post, Ts, u, -1); % t = -1 means we don't care about time
...
%initialisations
P_post = eye(6);
x_post = [r; 0; 0; 0; 0; 0]; % we follow a circle
...
% collect all the hypothetic control entries for each k
u_all = {};

%----------------initialise stuff for nlpsol--------------------
...
for k = 1:N-1
        ...
        w = {w{:}, uk};
        lbw = [lbw; [-100; -100]]; % input should be constrained to [-100%, 100%]
        ubw = [ubw; [100; 100]];
        w0 = [w0; [0; 0]];
...
for i = 1:Ns
    ...
    for k = 1:N-1
        ...
        fdk = fd('x0', xk, 'Ts', dt, 'p', uk,'t0', pred_t);
        xk = fdk.xf;
        J = J + fdk.qf;
    ...
    options.ipopt = struct('warm_start_init_point','yes');
    % options.ipopt = struct('linear_solver', 'ma_27');
    solver = nlpsol('solver', 'ipopt', nlp, options);
    % sol = solver('x0', w_warm,'lbg', lbg, 'ubg', ubg, 'lbx', lbw, 'ubx', ubw, 'lam_x0', lam_w_warm, 'lam_g0', lam_g_warm);
    sol = solver('x0', w0,'lbg', lbg, 'ubg', ubg, 'lbx', lbw, 'ubx', ubw, options);
    w_opt = full(sol.x);
    w_warm = w_opt; % this doesn't seem to work...
    lam_w_warm = full(sol.lam_x);
    lam_g_warm = full(sol.lam_g);
    % collect all the hypothetic control entries for each k
    u_all{i} = w_opt;
    ...
    [x_post, ~] = predict(x_post, P_post, dt, u);
    
    current_t = current_t + dt; % current time
...
x = cell2mat(x);
x = x';
time = linspace(0,10,Ns);

% see hypothetic shooting points point xi
hold off
idx = 1;
x_hypo = [];
u_hypo = u_all{idx};
...
xPrev = x_init;
for k=1:N-1
    xNext = fd(xPrev, dt, u_hypo(2*(k-1)+1:2*(k-1)+2), (idx-1)*dt);
    x_hypo = [x_hypo, xNext([1,2])];
    xPrev = xNext;
end

figure(1)
hold on
plot(r*cos(0.5 * time),r*sin(0.5 * time),'bo-')
plot(x(:,1),x(:,2),'m--');
plot(full(x_hypo(1,:)),full(x_hypo(2,:)),'o')